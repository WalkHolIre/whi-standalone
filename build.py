...
def fetch_translations(lang):
    ...
    return {
        'tours': {tt['tour_id']: tt for tt in (tour_trans or [])},
        'destinations': {dt['destination_id']: dt for dt in (dest_trans or [])},
    }
...
def apply_tour_translation(tour, translation):
    ...
    for field in translatable_fields:
        ...
        if val and val.strip():
            merged[field] = val
            # Also map meta_description -> seo_description if needed
...
def translate_html_ui(html, lang):
    ...
    This replaces known English static text in the HTML templates with the target language."""
    if lang == 'en':
        return html

    # Map of English text -> translated text for template static content
    # These are exact string replacements for text found in HTML templates
    replacements = {
        # Navigation
        '>Home<': f'>{t("home", lang)}<',
        ...
        # Common labels in render functions
...
def build_language_site(lang, tours, destinations, reviews, faqs, regions, posts,
    ...
    if lang == 'en':
        ...
        prefix_adjustment = f'../'  # for links back to shared assets
    ...
    for tour in tours:
        ...
        # For non-English: only build if translation exists
        ...
        # Apply destination translation if available
        ...
        # Apply translations to related tours too
        ...
        if html:
            # Post-process: translate UI strings
            html = translate_html_ui(html, lang)

            # Fix relative paths for language subdirectory
            if lang != 'en':
                # Adjust paths: ../images -> ../../images, etc.
                html = html.replace('href="../', 'href="../../')
                html = html.replace('src="../', 'src="../../')
                # Fix root-relative paths that need adjusting
                html = html.replace('href="/', f'href="/{lang}/')
                # But keep absolute URLs untouched (https://, //, etc.)
    ...
    log(f"Generated {lang_tour_count} {lang} tour pages")
    ...
    for destination in destinations:
        ...
        # For destinations, build even without translation (use English fallback)
        # but only if there are translated tours for this destination
        if lang != 'en':
        ...
        # Translate the tours shown on destination page
        ...
        if html:
            ...
            if lang != 'en':
                html = html.replace('href="tours/', f'href="tours/')  # keep same since we're in /de/
                html = html.replace('href="/', f'href="/{lang}/')
                html = html.replace('<html lang="en"', f'<html lang="{lang}"')
